...
void test_mult_gf16(void){
    ...
    printf("c[0] = %X = 137 and c[1] = %X = 200\n", c[0], c[1]);
    
};

void test_eval_poly_gf16(void){
    
    poly16 c0,c1,c2,x,res;
    c0[0] = 0x17;
    c0[1] = 0x36;
    c1[0] = 0x12;
    c1[1] = 0x20;
    ...
    poly_evaluation(pol,x,res);
    
    printf("c[0] = %X = B4 and c[1] = %X = 71\n", res[0], res[1]);
};
...
void test_prg(void){
    ...
    for (int j=0; j<degsize; j++) {
        if(j==deg){
            printf("coeff: %X and %X.\n", test.coeff[j][0], test.coeff[j][1]);
        } else{
            printf("coeff: %X and %X, ", test.coeff[j][0], test.coeff[j][1]);
        }
    }
    
    for (int i=0; i<10; i++) {
        printf("round%d: %X and %X\n",i, get_prg_value(&test), get_prg_value(&test));
    }
