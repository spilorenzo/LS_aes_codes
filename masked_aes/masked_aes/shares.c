//
//  shares.c
//  masked_aes
//
//  Created by Lorenzo SPIGNOLI on 16/05/2023.
//

#include "shares.h"

byte sq[256]={
};

byte A[256]={
};

byte invA[256]={
};

byte tsmult[1024]={
};

void copy_shares(shares input, shares output){
    
    // copy each share of input in output
    for (int i=0; i<n+1; i++) {
        output[i] = input[i];
    }
};

byte get_square(byte x){
    
    // return x^2
    return sq[x];
};

byte get_affine(byte x){
    
    // return Af(x)
    return A[x];
};

byte get_inv_affine(byte x){
    
    // return Af^-1(x)
    return invA[x];
};

void square_shares(shares input){
    
    // square each element of the state
    for (int i=0; i<n+1; i++) {
        input[i] = get_square(input[i]);
        
    }
    
};

byte multtable(byte x,byte y){
    
    // Computes z=x*y in GF(2^8) using four 8-bit tables
    return tsmult[0   | ((x & 15) << 4) | (y & 15)] ^
         tsmult[512 | (x & 240) | (y & 15)] ^
         tsmult[256 | ((x & 15) << 4) | (y >> 4)] ^
         tsmult[768 | (x & 240) | (y >> 4)];
};

void sec_mult(shares input_a, shares input_b, shares output){
    
    // define auxiliary variables
    byte rij, rji, aibj, ajbi;
    
    // compute masked multiplication from RP10
    // first c_i = a_ib_i
    for (int i=0; i<n+1; i++) {
        output[i] = multtable(input_a[i], input_b[i]);
    }
    
    // then r_ij and r_ji and update c_i and c_j accordingly
    for (int i=0; i<n+1; i++) {
        for (int j=i+1; j<n+1; j++) {
            rij = rand()%256;
            aibj = multtable(input_a[i], input_b[j]);
            ajbi = multtable(input_a[j], input_b[i]);
            rji = rij ^ aibj ^ ajbi;
                
            output[i] = output[i] ^ rij;
            output[j] = output[j] ^ rji;
        }
    }

};

void refreshmasks(shares input){

    // define auxiliary variable
    byte aux;

    // compute the refresh
    for (int i=0; i<n+1; i++) {
        for (int j=i+1; j<n+1; j++) {
            // refresh element by element and update the last share
            aux = rand()%256;
            input[i] = input[i] ^ aux;
            input[j] = input[j] ^ aux;
        }
    }
};

void sec_exp254(shares input){
    
    //define auxiliary shares
    shares z, y, y2, w;
    
    // z_i = x_i^2
    copy_shares(input,z);
    square_shares(z);
    // refresh z_i
    refreshmasks(z);
    // y_i = SecMult(z_i,x_i)
    sec_mult(input,z,y);
    // w_i = y_i^4
    copy_shares(y,w);
    square_shares(w);
    square_shares(w);
    // refresh w_i
    refreshmasks(w);
    // y_i = SecMult(y_i,w_i)
    sec_mult(y, w, y2);
    // y_i = y_i^16
    square_shares(y2);
    square_shares(y2);
    square_shares(y2);
    square_shares(y2);
    // y_i = SecMult(y_i,w_i)
    sec_mult(y2,w,y);
    // y_i = SecMult(y_i,z_i)
    sec_mult(y,z,input);
};

void sec_sbox(shares input){
    
    // securely compute x_i^254
    sec_exp254(input);
    
    // for each share compute the affine transformation
    for (int i=0; i<n+1; i++) {
        input[i] = get_affine(input[i]);
    }
    
    // in case n is odd add 0x63
    if (n%2 == 1) {
        input[0] = input[0] ^ 0x63;
    }
};

void sec_rsbox(shares input){
    
    // in case n is odd add 0x63
    if (n%2 == 1) {
        input[0] = input[0] ^ 0x63;
    }
    
    // for each share compute the affine^-1 transformation
    for (int i=0; i<n+1; i++) {
        input[i] = get_inv_affine(input[i]);
    }
    // securely compute x_i^254
    sec_exp254(input);
};
