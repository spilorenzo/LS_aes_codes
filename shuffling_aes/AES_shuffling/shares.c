//
//  shares.c
//  AES_shuffling
//
//  Created by Lorenzo SPIGNOLI on 20/02/21.
//

#include "shares.h"

byte sq[256]={
};

byte A[256]={
};

byte invA[256]={
};

byte tsmult[1024]={
};

void copy_shares(shares vct_input, shares vct_output){
    
    // copy each share of input in output
    for (int i=0; i<m+1; i++) {
        vct_output[i] = vct_input[i];
    }
};

byte square(byte x){
    
    // return x^2
    return sq[x];
};

byte affine(byte x){
    
    // return Af(x)
    return A[x];
};

byte inv_affine(byte x){
    
    // return Af^-1(x)
    return invA[x];
};

void square_shares(shares vct_x){
    
    // square each element of the state
    for (int i=0; i<m+1; i++) {
        vct_x[i] = square(vct_x[i]);
        
    }
    
};

byte mult(byte x,byte y){
    
    // define auxiliary variables
    byte z=0;
    byte a=128;
    
    // perform the (slow) multiplication over GF(2^8)
    for(int i=7; i>=0; i--){
        z=xtime(z);
        if((y & a)!=0) z = z^x;
        a = a>>1;
    }
    return z;
};

byte multtable(byte x,byte y){
    
    // Computes z=x*y in GF(2^8) using four 8-bit tables
    return tsmult[0   | ((x & 15) << 4) | (y & 15)] ^
         tsmult[512 | (x & 240) | (y & 15)] ^
         tsmult[256 | ((x & 15) << 4) | (y >> 4)] ^
         tsmult[768 | (x & 240) | (y >> 4)];
};

void sec_mult(shares vct_a, shares vct_b, shares vct_c){
    
    // define matrix for r_ij
    static unsigned char r[m+1][m+1];
    
    // compute the r_ij needed for a secure RP multiplication over GF(2^8)
    for (int i=0; i<m+1; i++) {
        for (int j=i+1; j<m+1; j++) {
            int rdm = rand()%256;
            r[i][j] = rdm;
            byte aibj = multtable(vct_a[i],vct_b[j]);
            byte ajbi = multtable(vct_a[j],vct_b[i]);
            r[j][i] = r[i][j] ^ aibj ^ ajbi;
        }
    }
    
    // compute the output shares
    for (int i=0; i<m+1; i++) {
        vct_c[i] = multtable(vct_a[i], vct_b[i]);
        for (int j=0; j<m+1; j++) {
            if (i == j) {
            }
            else {
                vct_c[i] = vct_c[i] ^ r[i][j];
            }
        }
    }

};

void refresh_masks(shares vct_x){

    // define auxiliary variable
    byte aux;

    // compute the refresh
    for (int i=0; i<m+1; i++) {
        aux = rand()%256;
        vct_x[m] = vct_x[m] ^ aux;
        vct_x[i] = vct_x[i] ^ aux;
    }
};

byte exp254(byte x){
    
    // define auxiliary variables
    byte z,y,w;
    // compute efficiently the RP x^254
    z = square(x);                             // x^2
    y = multtable(z, x);                             // x * x^2 = x^3
    w = square(square(y));                      // ((x^3)^2)^2 = x^12
    y = multtable(y, w);                             // x^3 * x^12 = x^15
    y = square(square(square(square(y))));      // ((((x^15)^2)^2)^2)^2 = x^240
    y = multtable(y, w);                             // x^240 * x^12 = x^252
    y = multtable(y, z);                             // x^252 * x^2 = x^254
    
    return y;
};

void sec_exp254(shares vct_x){
    
    //define auxiliary shares
    shares vct_z, vct_y, vct_w;
    
    // z_i = x_i^2
    copy_shares(vct_x, vct_z);
    square_shares(vct_z);
    // refresh z_i
    refresh_masks(vct_z);
    // y_i = SecMult(z_i,x_i)
    sec_mult(vct_x, vct_z, vct_y);
    // w_i = y_i^4
    copy_shares(vct_y, vct_w);
    square_shares(vct_w);
    square_shares(vct_w);
    // refresh w_i
    refresh_masks(vct_w);
    // y_i = SecMult(y_i,w_i)
    sec_mult(vct_y, vct_w, vct_y);
    // y_i = y_i^16
    square_shares(vct_y);
    square_shares(vct_y);
    square_shares(vct_y);
    square_shares(vct_y);
    // y_i = SecMult(y_i,w_i)
    sec_mult(vct_y, vct_w, vct_y);
    // y_i = SecMult(y_i,z_i)
    sec_mult(vct_y, vct_z, vct_y);
    
    // x_i^254
    copy_shares(vct_y, vct_x);
};

void sec_sbox(shares vct_x){
    
    // securely compute x_i^254
    sec_exp254(vct_x);
    
    // for each share compute the affine transformation
    for (int i=0; i<m+1; i++) {
        vct_x[i] = A[vct_x[i]];
    }
    
    // in case m is odd add 0x63
    if (m%2 == 1) {
        vct_x[m] = vct_x[m] ^ 0x63;
    }
};

void sec_rsbox(shares vct_x){
    
    // in case m is odd add 0x63
    if (m%2 == 1) {
        vct_x[m] = vct_x[m] ^ 0x63;
    }
    
    // for each share compute the affine^-1 transformation
    for (int i=0; i<m+1; i++) {
        vct_x[i] = invA[vct_x[i]];
    }
    
    // securely compute x_i^254
    sec_exp254(vct_x);
};

int main3(int argc, char *argv[]) {
    
    srand((unsigned int) time(0));
    
    byte a = 2;

    shares a_shares;
    a_shares[m] = a;
    
    
    for (int i=0; i<m; i++) {
        int rdm = rand()%256;
        a_shares[i] = rdm;
        a_shares[m] = a_shares[m] ^ a_shares[i];
    }
    
    printf("shares for a = ");
    for (int i=0; i<m+1; i++) {
        printf("%X ", a_shares[i]);
    }
    printf("\n");
    
    byte aux = 0;
    for (int i=0; i<m+1; i++) {
        aux = aux ^ a_shares[i];
    }
    printf("xored a shares = %X\n", aux);
    
    sec_exp254(a_shares);
    
    aux = 0;
    for (int i=0; i<m+1; i++) {
        aux = aux ^ a_shares[i];
    }
    printf("xored a shares = %X\n", aux);
    
    return 0;
};
